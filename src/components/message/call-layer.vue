<template>
	<div class="call-container" v-if="dialling || calling || isDialled">
		<div class="choose" v-if="isDialled">
			<div class="title">{{ toAccount }}&nbsp;来电话了</div>
			<div class="buttons">
		...
		<div class="call" v-if="dialling || calling">
			<div class="title" v-if="dialling">正在呼叫&nbsp;{{ toAccount }}...</div>
...
export default {
	...
	methods: {
		...
		async initTrtc(options) {
			// 初始化 trtc 进入房间
			this.Trtc = new RtcClient(options)
			await this.Trtc.createLocalStream({ audio: true, video: true })
				.then(() => {
					// 在进房之前，判断设备
					this.Trtc.join()
					this.ready = true
