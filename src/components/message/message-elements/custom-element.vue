<template>
	<message-bubble :isMine="isMine" :message="message">
		<div class="custom-element-wrapper">
			...
			<div v-else-if="JSON.parse(message.payload.data).messageType === 'RTC_EVENT_ANSWER'"><span class="media_msg">[接听]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType === 'ADD_FRIEND_MESSAGE'"><span class="media_msg">[请求添加好友]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType === 'SEE_WE_CHAT_MESSAGE'"><span class="media_msg">[查看微信号消息]</span></div>
			<div v-else-if="JSON.parse(message.payload.data).messageType === 'SEE_PHONE_MESSAGE'"><span class="media_msg">[查看手机号消息]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType === 'SEE_QQ_MESSAGE'"><span class="media_msg">[查看QQ号消息]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType === 'SEND_WE_CHAT_MESSAGE'"><span class="media_msg">[发送微信号消息]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType === 'SEND_PHONE_MESSAGE'"><span class="media_msg">[发送手机号]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType === 'SEND_QQ_MESSAGE'"><span class="media_msg">[发送QQ号消息]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType === 'REFUSE_FRIEND_APPLY_MESSAGE'"><span class="media_msg">[拒绝好友申请]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType === 'AGREE_FRIEND_APPLY_MESSAGE'"><span class="media_msg">[同意好友申请]</span></div>
		...
		<br/> -->
			...
			<div v-else-if="JSON.parse(message.payload.data).messageType == 'RTC_INTELLIGENT_EVENT_LAUNCH_AUDIO '"><span class="media_msg">[马甲语音通话]</span></div>
			<div v-else-if="JSON.parse(message.payload.data).messageType == 'RTC_EVENT_SHADE_VIDEO'"><span class="media_img">[事件，遮罩]</span></div>

			<div v-else-if="JSON.parse(message.payload.data).messageType == 'RTC_EVENT_LOCK_HINT'"><span class="media_msg">[事件，余额不足提示]</span></div>
